...
import { notify, useNotifications, useWindowManager } from "@benos/core";
import { useApp, useApps, useDesktopContext, useTheme } from "./desktop-context";
...
/**
 * Menu-bar text colors: `chrome.menuBarText` overrides the palette appearance
 * (macOS picks black or white bar labels from wallpaper luminance; the theme
 * declares which instead). Hex pairs keep the `${primary}1a` hover-fill
 * convention valid.
 */
function useMenuBarText(): { primary: string; secondary: string } {
  const theme = useTheme();
  switch (theme.chrome.menuBarText) {
    case "dark":
      return { primary: "#17171a", secondary: "#636366" };
    case "light":
      return { primary: "#f1f3f8", secondary: "#a6a9b0" };
    default:
      return {
      ...
      items: [
        {
          ...
          onSelect: () =>
            notify({
              title: brand,
              body: "An OS-style desktop, built with BenOS.",
              level: "info",
            }),
        },
        { separator: true },
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {showBrand && brand && (
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.background = `${bar.primary}1a`;
            }}
            onMouseLeave={(e) => {
        ...
        {!clockCentered &&
          theme.chrome.appMenus !== false &&
          focusedName &&
          (mode === "compact" ? (
            <span style={{ fontWeight: 600, color: bar.primary }}>{focusedName}</span>
          ) : (
            <AppMenus
              name={focusedName}
      ...
      {clockCentered && (
        ...
        >
          <SystemClock color={bar.primary} accent={theme.palette.accent} />
        </div>
      )}
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        {!clockCentered && (
          <SystemClock color={bar.secondary} accent={theme.palette.accent} />
        )}
      </div>
    </header>
  );
      ...
      {workspaces.map((id, idx) => {
        ...
        return (
          <Tooltip
            key={id}
            text={
              layout?.workspaces?.find((w) => w.id === id)?.name ??
              `Workspace ${String(idx + 1)}`
            }
            ...
            >
              <span
                ...
                style={{
                  ...
                  height: 6,
                  borderRadius: 3,
                  background: isActive ? theme.palette.accent : bar.secondary,
