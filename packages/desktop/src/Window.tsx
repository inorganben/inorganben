...
import type { OpenWindow } from "@benos/core";
...
function setGenieVars(
  ...
  const dockRect = getDockTileRect(tileKey);
...
export function Window({ win, hidden = false }: WindowProps) {
  ...
  // The dock tile this window minimizes into: the app tile, or the transient
  // system tile keyed `system:<id>` (matches Dock's `data-dock-app-id`).
  const dockTileKey =
    win.payload.kind === "app" ? win.payload.appId : `system:${win.payload.systemId}`;
  const systemDef =
  ...
  useLayoutEffect(() => {
    ...
    if (elRef.current) setGenieVars(elRef.current, dockTileKey, genieScale);
  ...
  }, [win.state, dockTileKey, genieMs, genieScale]);
  ...
  // Per-app frame overrides (app.chrome > theme > default).
  const appChrome = app?.chrome;
  const CustomTitleBar =
    appChrome?.titleBar && appChrome.titleBar !== "hidden"
      ? appChrome.titleBar
      : undefined;
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage: appChrome?.background
          ? `url("${appChrome.background}")`
          : undefined,
        backgroundSize: "cover",
        backgroundPosition: "center",
        backdropFilter: theme.blur.surface,
        WebkitBackdropFilter: theme.blur.surface,
        border: appChrome?.border ?? `1px solid ${theme.palette.border}`,
        borderRadius: maximized ? 0 : (appChrome?.radius ?? theme.shape.windowRadius),
        boxShadow: focused
    ...
    >
      {appChrome?.titleBar === "hidden" ? null : CustomTitleBar ? (
        <CustomTitleBar
          title={title}
          focused={focused}
          maximized={maximized}
...
/**
 * The theme's standard control cluster (traffic lights, Windows caption
 * buttons, GNOME headerbar buttons, or the minimal close). Shared between the
 * built-in `TitleBar` and the `controls` slot handed to a custom title bar.
 */
function WindowControls({
  ...
  return <MinimalControls focused={focused} onClose={onClose} />;
}
