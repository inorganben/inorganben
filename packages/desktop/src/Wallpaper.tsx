...
import { getPath, useWindowManager } from "@benos/core";
import { useDesktopContext, useSettings, useTheme } from "./desktop-context";
...
export function Wallpaper() {
  ...
  // Per-workspace override from the layout config (macOS Spaces / Windows
  // virtual desktops each carry their own picture); the theme default
  // applies to workspaces without one. An explicit Settings choice wins over
  // both: the layout pictures are backend defaults, the user's pick is a
  // runtime override.
  const workspaceWallpaper = layout?.workspaces?.find(
    (w) => w.id === state.activeWorkspaceId,
  )?.wallpaper;
  const userPicked = getPath(prefs, "wallpaper.src") !== undefined;
  const src = userPicked ? wallpaper.src : (workspaceWallpaper ?? wallpaper.src);
