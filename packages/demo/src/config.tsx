 ...
 * here is parameterized by `assetBase` ("/" for the playground, "/inorganben/"
 * for the docs deploy) since each app serves its own `public/` under a
 * different path.
 */
import type { ComponentType } from "react";
import { useState } from "react";
import type { App, OsTheme } from "@benos/core";
import { getSystemWindow, registerSystemWindow } from "@benos/desktop";
import { createMacosTheme } from "@benos/theme-macos";
import { createUbuntuTheme } from "@benos/theme-ubuntu";
import { createWindowsTheme } from "@benos/theme-windows";

export type DemoThemeChoice = "macos" | "windows" | "ubuntu" | "benos";

const THEME_STORAGE_KEY = "rui-os:playground-theme";

export function isThemeChoice(value: string | null): value is DemoThemeChoice {
  return (
    value === "macos" || value === "windows" || value === "ubuntu" || value === "benos"
  );
}

// `?theme=` wins (a shareable deep link), then the visitor's last choice, then BenOS.
export function readInitialThemeChoice(): DemoThemeChoice {
  if (typeof window === "undefined") return "benos";
  ...
  return function PngIcon({ size = 24 }: { size?: number }) {
    return (
      <img
        ...
        alt=""
        decoding="async"
        // Non-square art (e.g. portrait Tux) letterboxes instead of squashing.
...
export function demoWallpapers(assetBase: string): { src: string; label: string }[] {
  return [
    ...
    { src: withBase(assetBase, "benos-wallpaper.jpg"), label: "BenOS" },
  ];
}
...
export function buildDemoTheme(choice: DemoThemeChoice, assetBase: string): OsTheme {
  ...
  if (choice === "benos") {
    const base = createMacosTheme({
      wallpaperSrc: withBase(assetBase, "benos-wallpaper.jpg"),
      darkWallpaperSrc: withBase(assetBase, "benos-wallpaper.jpg"),
      wallpaperOptions,
      liquidGlass: true,
    });
    return {
      ...
      id: "benos",
      name: "BenOS",
      chrome: { ...base.chrome, appMenus: false, menuBarText: "dark" },
    };
  }
