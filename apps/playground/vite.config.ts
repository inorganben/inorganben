 ...
 */
// GitHub Pages serves project sites under /<repo>/. In Actions,
// GITHUB_REPOSITORY is "owner/repo", so the deploy needs no per-repo config;
// a PAGES_BASE override covers the user-site case (set PAGES_BASE=/ when the
// repo is <username>.github.io). Local dev and every other build stay at /.
const repoPagesBase = process.env.GITHUB_REPOSITORY?.split("/")[1];
const pagesBase =
  process.env.PAGES_BASE ?? (repoPagesBase ? `/${repoPagesBase}/` : "/");
